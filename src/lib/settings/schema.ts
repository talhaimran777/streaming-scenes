...
export const globalSettingsSchema = z.object({
  ...
  viewerCountSource: z.enum(["youtube", "external"]).default("youtube"),
  latestFollowerSource: z.enum(["youtube", "external"]).default("youtube"),
  showExternalSlotGuides: z.boolean().default(false),
...
export const justChattingSchema = z.object({
  ...
  externalViewerSlotWidthPx: z.number().int().min(0).max(400).default(72),
  externalFollowerSlotWidthPx: z.number().int().min(0).max(800).default(280),
  externalFollowerSlotHeightPx: z.number().int().min(0).max(200).default(38),
